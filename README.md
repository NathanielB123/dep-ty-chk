...
- It is worth noting that this development focusses on implicit type substitutions paired with explicit term substitutions. Making substitutions on both types and terms fully computational here is impossible for the above-mentioned issues with circularity, but I believe it *is* possible to make substitutions compute on both terms AND *most* type constructors by having every type constructor that embeds terms store substitutions applied to the contained term.
